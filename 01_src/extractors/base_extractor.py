...
class BaseExcelExtractor(ABC):
    ...
    def _extract_section(
        ...
        try:
            ...
            self.logger.info(f"Processing structure: {structure}")
            for level_1, level_2_items in structure.items():
                ...
                if level_1_row != -1:
                    ...
                    # For each level 2 item under this level 1
                    for level_2 in level_2_items:
                        ...
                        if level_2_row != -1:
                            ...
                            # Find the first non-empty cell in the row (should be our target text)
                            row_data = df.iloc[level_2_row]
                            start_col = None
                            
                            for col_idx, value in enumerate(row_data):
                                if isinstance(value, str) and value.strip() == level_2.strip():
                                    start_col = col_idx
                                    break
                            
                            if start_col is not None:
                                # Extract values from adjacent columns
                                value_2022 = df.iloc[level_2_row, start_col + 1]
                                value_2023 = df.iloc[level_2_row, start_col + 2]
                                abweichung = df.iloc[level_2_row, start_col + 3]
                                ...
                                self.logger.debug(f"Extracted values: 2022={value_2022}, 2023={value_2023}, abweichung={abweichung}")
